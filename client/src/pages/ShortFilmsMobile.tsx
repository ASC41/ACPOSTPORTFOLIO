...
export default function ShortFilmsMobile() {
  const [selectedItemId, setSelectedItemId] = useState<string | null>(null);

  // Filter for short films
  const shortFilmItems = portfolioData.filter(item => 
    item.categories.includes("Short Films")
  );
      ...
      <section className="relative h-[40vh] bg-gradient-to-b from-gray-800 to-gray-900 flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto">
            Narrative storytelling through cinematic post-production
          </p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {shortFilmItems.length === 0 && (
            <div className="text-center py-12">
              <p className="text-white/60">No short films available.</p>
            </div>
          )}
        </div>
      </section>
